	...
	for _, phase := range []string{"plan", "apply"} {
		for _, symbolic := range []string{"branch", "dangling_branch", "archive_branch", "archive_other", "dangling_archive"} {
			t.Run(phase+"/"+symbolic, func(t *testing.T) {
				...
				plan, err := PlanMirrorPublicationReconciliation(ctx, gateDir, work, "feature", liveHead, RunOwnedHeads{Submitted: privateHead})
				...
				if phase == "plan" {
					_, err = PlanMirrorPublicationReconciliation(ctx, gateDir, work, "feature", liveHead, RunOwnedHeads{Submitted: privateHead})
				} else {
