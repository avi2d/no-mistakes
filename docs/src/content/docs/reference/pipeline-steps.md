...
- When the local gate mirror exists, applies the [private mirror reconciliation contract](/no-mistakes/concepts/gate-model/#private-mirror-reconciliation), including Decision 41-A for the exact submitted head and the run's own last published head, while preserving newer descendants through shared worktree refs; skips a missing mirror
...
- On CI failure: fetches failed job logs only for the selected checks, sends them to the agent with user intent when available, and, if the agent produces changes, commits them with [`commit.fix_message`](/no-mistakes/reference/global-config/#commitfix_message).
  GitHub logs come from `gh run view --log-failed`, GitLab logs from `glab ci trace`, Forgejo logs from the exact native check target plus `forgejo-axi run view --log-failed` when runtime routes are available, and Bitbucket Cloud logs from failed pipeline step logs.
  Azure DevOps has no first-class build-log command, so the agent fixes from the failing-check list without logs.
  Target-aware providers return each selected check's evidence separately.
  The prompt labels that evidence with the check and provider identity, shares a 32 KiB budget across all selected targets, and marks per-target truncation or incomplete retrieval explicitly, so one check cannot hide another's missing evidence.
  Every CI repair path, including a merge-conflict-only repair, follows the same invariant-complete discipline as the Review fixer and the shared removal-first rule described above.
  That discipline has four parts: state the invariant the failure exposes, close it at every sibling site in the changed area in the same round with the same small correction or one shared boundary, prefer a deeper root cause and simplification over machinery for the symptoms, and re-trace the failing sequence and the ordinary path through every changed function before verifying.
  The agent may conclude that no code change is warranted when the PR's code did not cause the red check.
  Examples are a stale run, an infrastructure or attestation check like `PR must be raised via no-mistakes` that fails only because a later pipeline push moved the head, and any failure external to the code.
  What happens next follows one rule on every CI-fix path.
  A repair is published without revalidating only when its continuity with the reviewed, published head can be proven, meaning the repaired head is the run's review-approved commit or a descendant of it.
  A provable repair is published immediately through the Push step's own guarded publication path, using that step's own remote-safety decision, and the monitor keeps watching the same run.
  On supported providers, an existing pipeline attestation is rebound to the proposed head before the branch push, and failure to settle that rewrite after retries reports the repair as unsettled without pushing.
  An unavailable SCM host skips the rewrite, while strict attestation head equality keeps any stale binding fail-closed.
  Anything else is held locally, the run's review approval is revoked, and validation restarts from Review so Push republishes it only after Review approves it.
  [`ci.revalidate_repairs`](/no-mistakes/reference/repo-config/#cirevalidate_repairs) sets the intent identically on every path.
  The default `false` publishes when it is provable, and `true` revalidates outright.
  Under the default [`rebase.strategy: rebase`](/no-mistakes/reference/repo-config/#rebasestrategy), a merge-conflict repair rebases, so its continuity is never provable and it always revalidates.
  Under `rebase.strategy: merge`, a merge-conflict repair merges the base into the published head, so its continuity is plain ancestry and it follows the same rule as any other repair.
  Forgejo status gating remains active when logs are unsupported or unavailable
...
- Under [`rebase.strategy: merge`](/no-mistakes/reference/repo-config/#rebasestrategy), merges the latest PR base branch tip into the branch itself with `git merge --no-ff` instead of asking the agent to rebase.
  The agent's prompt names the conflicted files, carries the Rebase step's additive-resolution rule, and tells the agent to conclude the merge with `git commit --no-edit`.
  When the merge applies without conflicts and the conflict was the round's only target, no agent runs.
  After the agent finishes, the step checks the same merge shape the Rebase step checks, and resets the run worktree to the head the merge started from when that check fails.
  Any CI repair under `merge` whose head does not contain the run's published head is refused before it is recorded or pushed, so an open PR's branch is appended to and never rewritten.
  The refusal covers a head that a timed-out agent committed, and the run worktree is reset to the run's recorded head instead of keeping the refused head checked out
